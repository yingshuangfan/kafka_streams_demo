...
public class HoppingWindowWordCount {
    ...
    public static void main(String[] args) throws Exception, IllegalArgumentException {
        ...
        // TODO: 对时间窗聚合
        long windowSizeMs = TimeUnit.MINUTES.toMillis(5); // 1 * 60 * 1000L(ms)
        long hoppingSizeMs = TimeUnit.MINUTES.toMillis(1); // 1 * 60 * 1000L(ms)
        final TimeWindowedKStream<String, String> windowedGroupedWords = groupedWords.windowedBy(TimeWindows.of(windowSizeMs).advanceBy(hoppingSizeMs));
        final KTable<Windowed<String>, Long> wordsCountPer5Minutes = windowedGroupedWords.aggregate(
                () -> 0L,
        ...
        // TODO: 聚合结果console展示
        final KStream<String, Long> wordsCountStream = wordsCountPer5Minutes.toStream((windowedWord, count) -> windowedWord.toString());
